...
int q(int c, int x, int y, int z, int k, int l, int m, int a, int b) {
    ...
    a = -x * k / M - y * l / M - z * m / M;
    p = ((b = a * a / M - b) >= 0 ? (I(b * M, colours, 0), b = E, a + (a > b ? -b : b)): -1.0);
    return 0;
}
...
int gradient_fill(int e, int f, int g, int h, int i, int j, int d, int a, int b, int V) {
    ...
    //Think this does something with green
    //T = T < 0 ? 0 : T > O ? O : T;
    if (T<0) {
        T=0;
    } else {
    ...
    //Think this does something with blue
    //U = U < 0 ? 0 : U > O ? O : U;
    if (U<0) {
        U=0;
    } else {
...
int r(int x, int y) {
    R = G = B = 0;
    t(x, y, 0, 0);
    x < horizontal ? (printf("%c%c%c", R / antialias_factor / antialias_factor, G / antialias_factor / antialias_factor, B / antialias_factor / antialias_factor), r(x + 1, y)) : 0;
    return 0;
}
